...
class oddSpinBox(QSpinBox):
    def focusOutEvent(self, *args, **kwargs):
        ...
        if val % 2 == 0:
            self.setValue(val + 1)
        return QSpinBox.focusOutEvent(self, *args, **kwargs)
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        spacerItem = QSpacerItem(88, 37, QSizePolicy.Minimum, QSizePolicy.Expanding)
        ...
        self.featureSpins = [self.featureSize1_Spin, self.featureSize2_Spin, self.featureSize3_Spin,
                             self.featureSize4_Spin, self.featureSize5_Spin]
        self.featureLabels = [self.featureSize1_Label, self.featureSize2_Label, self.featureSize3_Label,
                              self.featureSize4_Label, self.featureSize5_Label]
    ...
    def run(self):
        ...
        try:
            ...
            QMessageBox.warning(self.centralwidget, 'Error', traceback.format_exc())
    ...
    def openImage(self):
        ...
        if np.array(ims).size == nZ * nT * nCh * ims[0].size:
            if order == 'czt':
                ims = np.reshape(ims, (nT, nZ, nCh, ims[0].shape[0], ims[0].shape[1])).astype(np.float)
            else:
                ims = np.reshape(ims, (nT, nCh, nZ, ims[0].shape[0], ims[0].shape[1])).astype(np.float)
                ims = np.swapaxes(ims, 1, 2)
        else:
            self.statusBar.showMessage(
                'Error: number of images (or sizes) does not correspond to z={0}, t={1}, ch={2}'.format(nZ, nT, nCh))
            raise Exception(
                'Error: number of images (or sizes) does not correspond to z={0}, t={1}, ch={2}'.format(nZ, nT, nCh))
        return ims
